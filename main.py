...
import numpy as np
from fitnes_fun import fitnes_fun
...
# particles_v = np.array([[0.65061274, -0.15857199, -0.62260656, -0.63292306, -0.73638317],
...
# #
# particles_x = np.array([[-0.15309862, -3.51241576, 1.63250581, -1.78929636, -1.65019358],
...
p_best = particles_x
p_best_val = f_val
index = np.argmin(f_val)
g_best = np.copy(particles_x[index])
g_best_val = np.copy(f_val[index])

r = rand(D)
rr = rand(D)
# r = np.array([0.33784365, 0.25767638, 0.24743797, 0.21011771, 0.32126972])
# rr = np.array([0.98155284, 0.19163743, 0.51977101, 0.34105693, 0.53782269])
for i in range(iter):
    for j in range(swarm_size):
        for k in range(D):
            r1 = r[k]
            r2 = rr[k]
            particles_v[j, k] = w * particles_v[j, k] + c1 * r1 * (p_best[j, k] - particles_x[j, k]) + c2 * r2 * (
                    g_best[k] -
                    particles_x[j, k])
    ...
    for j in range(swarm_size):
        ...
        if f_val[j] < p_best_val[j]:
            p_best[j] = np.copy(particles_x[j])
            p_best_val[j] = np.copy(f_val[j])

        if f_val[j] < g_best_val:
            g_best = np.copy(particles_x[j, :])
            g_best_val = np.copy(f_val[j])
